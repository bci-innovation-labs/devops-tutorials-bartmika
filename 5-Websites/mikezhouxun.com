...
1. Update your nginx config
    ...
    ```
    $ sudo vi /etc/nginx/sites-available/mikezhouxun.com.conf;
    ```
...
1. Remove our sample folder.
    ...
    ```
    $ rm /var/www/mikezhouxun.com/html/index.html
    ```

2. Copy (including keeping the permissions) using ``rsync``.

    ```
    $ sudo rsync -avP ~/wordpress/ /var/www/mikezhouxun.com/html/
    ```

3. Create our uploads folder.

    ```
    $ mkdir /var/www/mikezhouxun.com/html/wp-content/uploads
    ```
...
4. Grant permission.
    ...
    $ semanage fcontext -a -t httpd_sys_content_t "/var/www/mikezhouxun.com(/.*)?"
    ```
...
1. Run the following.
    ...
    ```
    cd /var/www/mikezhouxun.com/html
    ```
...
2. Copy our sample configuration AND grant permission.
    ...
    sudo chown -R nginx:nginx /var/www/mikezhouxun.com/html/*
    ```
...
5. Open up your browser and go to ````
    ...
    ```
    http://mikezhouxun.com/index.php
    ```
